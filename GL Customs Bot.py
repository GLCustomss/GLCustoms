...
PLINKO_MULTIPLIERS = [6.5, 3.0, 1.85, 1.18, 0.94, 0.78, 0.94, 1.18, 1.85, 3.0, 6.5]
...
def generate_crash_point():
    ...
    if roll < 0.25:
        crash_point = random.uniform(1.22, 1.78)
    elif roll < 0.57:
        crash_point = random.uniform(1.79, 3.45)
    elif roll < 0.85:
        crash_point = random.uniform(3.46, 7.20)
    elif roll < 0.97:
        crash_point = random.uniform(7.21, 15.50)
    else:
        crash_point = random.uniform(15.51, 40.00)
    ...
    try:
        ...
        if chosen_number is not None:
            ...
            if choice_text == result_colour:
                won = True
                payout_mult = 2.42 if choice_text in {"red", "black"} else 19.0
...
async def dice(ctx, bet_amount=None, side=None):
    ...
    try:
        ...
        payout = int(round(bet_amount * 2.08)) if won else 0
        new_balance = add_user_balance(ctx.author.id, payout)
        await check_casino_car_pack_role(ctx.author, ctx)
        embed = discord.Embed(
...
async def baccarat(ctx, bet_amount=None, side=None):
    ...
    try:
        ...
        won = side == outcome
        payout_mult = {"player": 2.18, "banker": 2.1, "tie": 9.7}[outcome] if won else 0.0
        payout = int(round(bet_amount * payout_mult))
        new_balance = add_user_balance(ctx.author.id, payout)
        await check_casino_car_pack_role(ctx.author, ctx)
