...
async def give(ctx, *, target_and_amount: str = None):
    ...
    lowered_amount = str(amount_raw).strip().lower()
    ...
    if ctx.author.id == OWNER_ID:
        try:
            ...
            amount = None
    elif has_security_permission(ctx.author):
        if lowered_amount in {"all", "max", "allin", "all-in", "inf", "infinite", "infinity", "∞"}:
            amount = None
        else:
            try:
    ...
    if ctx.author.id != OWNER_ID and not has_security_permission(ctx.author) and amount > MAX_GIVE_AMOUNT:
...
async def admincash(ctx, member: discord.Member = None, amount=None):
    ...
    if raw_amount in {"inf", "infinite", "infinity", "∞"}:
        confirmed = await confirm_action(
            ctx,
            title="Confirm Admin Cash",
            prompt_text=f"You are about to give **infinite cash** to {member.mention}."
        )
        if not confirmed:
            return
        ...
        embed = discord.Embed(
            title="Admin Cash",
            description=f"✅ Set {member.mention}'s balance to **infinite cash**.\n\n**New Balance:** ∞ 💵",
            colour=0x57F287,
            timestamp=now_dt()
        )
        embed.set_footer(text="GL Customs")
...
async def rewards(ctx):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "**Casino Commands**\n"
            "`!balance`, `!bal`, `!beg`, `!work`, `!give`, `!slots`, `!roulette`, `!crash`, `!rocket`, `!dice`, `!baccarat`, `!plinko`, `!mines`, `!mine`, `!bj`, `!blackjack`, `!leaderboard`, `!mystats`, `!rank`, `!profit`\n\n"
            "**Mines Jackpot**\n"
...
class AdvancedHelpView(discord.ui.View):
    ...
    def build_utility_embed(self):
        ...
        embed.add_field(
            ...
            value=(
                ...
                "`!vehiclehelp` — Open vehicle compatibility and install help\n"
    ...
    def build_vehicle_embed(self):
        ...
        embed.add_field(
            name="How to Install",
            value=(
                "Drag and drop the vehicle folder into your server's **resources** folder, "
                "then make sure the resource name matches the folder name."
            ),
            inline=False
        )
        embed.add_field(
            ...
            value=(
                "Start the vehicle through **txAdmin** by using `ensure foldername` in your server config "
                "or by starting the resource from the txAdmin console."
            ),
            inline=False
        )
        embed.add_field(
