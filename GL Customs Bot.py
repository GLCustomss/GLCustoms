    ...
    try:
        ...
        result_embed = discord.Embed(
            ...
            description=(
                f"{' | '.join(spin)}\n\n"
                f"{result_line}\n"
                f"└ Won: **{format_result_cash(payout, new_balance)} 💵**\n"
...
class MinesView(discord.ui.View):
        ...
        if index in self.mine_positions:
            ...
            if protected_pick:
                ...
                embed = self.build_embed(
                    ...
                    description=f"You lost {format_result_cash(round_mines_display_amount(self.bet_amount), get_user_balance(self.author.id))} 💵",
                    colour=0xED4245
                )
                await interaction.response.edit_message(embed=embed, view=self)
                return
    ...
    async def cash_out(self, interaction):
        ...
        embed = self.build_embed(
            ...
            description=f"You won {format_result_cash(round_mines_display_amount(total_payout), get_user_balance(self.author.id))} 💵\n\n**Balance:** {format_cash(get_user_balance(self.author.id))} 💵",
            colour=0x57F287
        )
        await interaction.response.edit_message(embed=embed, view=self)
...
class BlackjackView(discord.ui.View):
    ...
    async def settle_game(self, interaction):
        ...
        if dealer_total > 21 or player_total > dealer_total:
            ...
            result = f"✅ You won **{format_result_cash(payout, get_user_balance(self.author.id))} 💵**\n└ Profit: **{format_result_cash(payout - self.current_bet, get_user_balance(self.author.id))} 💵**"
            colour = 0x57F287
        elif player_total == dealer_total:
            add_user_balance(self.author.id, self.current_bet)
            await check_casino_car_pack_role(self.author, None)
            result = f"🟨 Push — returned **{format_result_cash(self.current_bet, get_user_balance(self.author.id))} 💵**"
            colour = 0xFEE75C
        else:
            result = f"❌ You lost **{format_result_cash(self.current_bet, get_user_balance(self.author.id))} 💵**"
            colour = 0xED4245
        self.finished = True
        self.finish_buttons()
        ACTIVE_GAME_USERS.discard(self.author.id)
    ...
    async def hit_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if blackjack_hand_total(self.player_hand) > 21:
            ...
            self.finish_buttons()
            ACTIVE_GAME_USERS.discard(self.author.id)
            embed = self.build_embed(reveal_dealer=True, result_text=f"❌ You busted and lost **{format_result_cash(self.current_bet, get_user_balance(self.author.id))} 💵**", result_colour=0xED4245)
            await interaction.response.edit_message(embed=embed, view=self)
            return
    ...
    async def exit_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        self.finish_buttons()
        ACTIVE_GAME_USERS.discard(self.author.id)
        embed = self.build_embed(reveal_dealer=True, result_text=f"🟦 Game exited — returned **{format_result_cash(self.current_bet, get_user_balance(self.author.id))} 💵**", result_colour=0x5865F2)
        await interaction.response.edit_message(embed=embed, view=self)
            ...
            if self.current_multiplier >= self.crash_point:
                ...
                embed = self.build_embed(
                    ...
                    description=(
                        ...
                        f"└ Lost: **{format_result_cash(self.bet_amount, get_user_balance(self.author.id))} 💵**\n"
