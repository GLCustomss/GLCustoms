...
def preview_bet_amount(bet_amount):
    try:
        value = int(bet_amount)
    except Exception:
        return 0
    if is_infinite_cash(value):
        return INFINITE_PREVIEW_BET
    return value


def format_preview_cash(amount):
    return format_cash(preview_bet_amount(amount))
...
def has_give_cap_bypass(member):
    if member.guild.owner_id == member.id:
        return True
    role_ids = {role.id for role in member.roles}
    return SECURITY_TEAM_ROLE_ID in role_ids or member.id == OWNER_ID
...
async def slots(ctx, bet_amount=None):
    ...
    try:
        ...
        display_payout = int(round(preview_bet_amount(bet_amount) * payout_mult))
        ...
        message = await send_game_premessage(
            ctx,
            f"Slots - {format_preview_cash(bet_amount)} 💵",
            "🎰 Booting slot machine...",
            "⚙️ Aligning reels..."
        )

        spin_embed = discord.Embed(
            title=f"Slots - {format_preview_cash(bet_amount)} 💵",
            description="🎰 | 🎰 | 🎰\n\n**Spinning...**",
            colour=0x5865F2,
            timestamp=now_dt()
        )
        ...
        result_embed = discord.Embed(
            ...
            description=(
                f"{' | '.join(spin)}\n\n"
                f"{result_line}\n"
                f"└ Won: **{format_cash(display_payout)} 💵**\n"
...
async def roulette(ctx, bet_amount=None, choice=None):
    ...
    try:
        ...
        for idx, shown in enumerate(spin_sequence):
            ...
            spin_embed = discord.Embed(
                title=f"Roulette - {format_preview_cash(bet_amount)} 💵",
                description=(
                    "🎡 Wheel spinning...\n\n"
...
class CrashGameView(discord.ui.View):
    ...
    def build_embed(self, final_text=None, colour=0x5865F2):
        ...
        embed = discord.Embed(
            title=f"Crash - {format_preview_cash(self.bet_amount)} 💵",
            description=desc,
            colour=colour,
            timestamp=now_dt()
        )
    ...
    async def cashout_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        display_payout = int(round(preview_bet_amount(self.bet_amount) * self.current_multiplier))
...
async def dice(ctx, bet_amount=None, side=None):
    ...
    try:
        ...
        display_payout = int(round(preview_bet_amount(bet_amount) * 2.15)) if won else 0
        new_balance = add_user_balance(ctx.author.id, payout)
        record_house_profit("dice", bet_amount, payout, ctx.author.id)
        await check_casino_car_pack_role(ctx.author, ctx)
        embed = discord.Embed(
            title=f"Dice - {format_preview_cash(bet_amount)} 💵",
...
async def baccarat(ctx, bet_amount=None, side=None):
    ...
    try:
        ...
        payout = int(round(bet_amount * payout_mult))
        display_payout = int(round(preview_bet_amount(bet_amount) * payout_mult)) if won else 0
        new_balance = add_user_balance(ctx.author.id, payout)
        record_house_profit("baccarat", bet_amount, payout, ctx.author.id)
        await check_casino_car_pack_role(ctx.author, ctx)

        embed = discord.Embed(
            title=f"Baccarat - {format_preview_cash(bet_amount)} 💵",
            description=(
...
class MinesView(discord.ui.View):
    ...
    async def reveal_tile(self, interaction, index):
        ...
        if index in self.mine_positions:
            ...
            if protected_pick:
                ...
                embed = self.build_embed(
                    ...
                    description=f"You lost {format_cash(round_mines_display_amount(preview_bet_amount(self.bet_amount)))} 💵",
                    colour=0xED4245
                )
                await interaction.response.edit_message(embed=embed, view=self)
                return
        ...
        current_cashout = round_mines_display_amount(max(self.bet_amount, int(self.bet_amount * current_multiplier)))
        embed = self.build_embed(
            description=(
                f"Avoid the mines hidden in the field to win.\n\n"
                f"**Mines:** {self.mine_count}\n"
                f"**Safe Gems Found:** {self.safe_hits}\n"
                f"**Cash Out Value:** {format_cash(round_mines_display_amount(max(preview_bet_amount(self.bet_amount), int(preview_bet_amount(self.bet_amount) * current_multiplier))))} 💵"
            ),
            colour=0xF1C40F
        )
        await interaction.response.edit_message(embed=embed, view=self)
    ...
    async def cash_out(self, interaction):
        ...
        display_payout = max(preview_bet_amount(self.bet_amount), int(preview_bet_amount(self.bet_amount) * mines_multiplier(self.safe_hits, self.mine_count, self.total_tiles)))
        total_payout = payout + self.jackpot_bonus
        display_total_payout = display_payout + self.jackpot_bonus
        ...
        embed = self.build_embed(
            ...
            description=f"You won {format_cash(round_mines_display_amount(total_payout))} 💵\n\n**Balance:** {format_cash(get_user_balance(self.author.id))} 💵",
            colour=0x57F287
        )
        await interaction.response.edit_message(embed=embed, view=self)
...
class BlackjackView(discord.ui.View):
    ...
    def build_embed(self, reveal_dealer=False, result_text=None, result_colour=0xF1C40F):
        ...
        embed = discord.Embed(
            title=f"Blackjack - {format_preview_cash(self.current_bet)} 💵",
    ...
    async def settle_game(self, interaction):
        ...
        if dealer_total > 21 or player_total > dealer_total:
            ...
            display_payout = int(round(preview_bet_amount(self.current_bet) * 2.1))
            add_user_balance(self.author.id, payout)
            await check_casino_car_pack_role(self.author, None)
            result = f"✅ You won **{format_cash(display_payout)} 💵**\n└ Profit: **{format_cash(display_payout - preview_bet_amount(self.current_bet))} 💵**"
            colour = 0x57F287
        elif player_total == dealer_total:
            add_user_balance(self.author.id, self.current_bet)
            await check_casino_car_pack_role(self.author, None)
            result = f"🟨 Push — returned **{format_preview_cash(self.current_bet)} 💵**"
            colour = 0xFEE75C
        else:
            result = f"❌ You lost **{format_preview_cash(self.current_bet)} 💵**"
            colour = 0xED4245
        self.finished = True
        self.finish_buttons()
        ACTIVE_GAME_USERS.discard(self.author.id)
    ...
    async def hit_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if blackjack_hand_total(self.player_hand) > 21:
            ...
            self.finish_buttons()
            ACTIVE_GAME_USERS.discard(self.author.id)
            embed = self.build_embed(reveal_dealer=True, result_text=f"❌ You busted and lost **{format_preview_cash(self.current_bet)} 💵**", result_colour=0xED4245)
            await interaction.response.edit_message(embed=embed, view=self)
            return
    ...
    async def exit_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        self.finish_buttons()
        ACTIVE_GAME_USERS.discard(self.author.id)
        embed = self.build_embed(reveal_dealer=True, result_text=f"🟦 Game exited — returned **{format_preview_cash(self.current_bet)} 💵**", result_colour=0x5865F2)
        await interaction.response.edit_message(embed=embed, view=self)
...
async def give(ctx, *, target_and_amount: str = None):
    ...
    amount_text = str(amount_raw).strip().lower()
    if has_give_cap_bypass(ctx.author):
        if ctx.author.id != OWNER_ID and amount_text in {"all", "max", "allin", "all-in", "inf", "infinite", "infinity", "∞"}:
            amount = None
        else:
            try:
                amount = resolve_bet_amount(ctx.author.id, amount_raw) if ctx.author.id == OWNER_ID else parse_cash_amount(amount_raw)
            except Exception:
                amount = None
    else:
        try:
    ...
    if amount is None:
        embed = discord.Embed(
            title="Give",
            description="❌ Invalid amount. Use values like `100000`, `100k`, `5m`, or `1b`.",
            colour=0xE74C3C,
            timestamp=now_dt()
        )
        embed.set_footer(text="GL Customs", icon_url=LOGO_URL)
    ...
    if not has_give_cap_bypass(ctx.author) and amount > MAX_GIVE_AMOUNT:
...
async def rewards(ctx):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "**Casino Commands**\n"
            "`!slots`, `!roulette`, `!crash`, `!rocket`, `!dice`, `!baccarat`, `!plinko`, `!mines`, `!bj`, `!profit`\n\n"
            "**Mines Jackpot**\n"
...
async def plinko(ctx, bet_amount=None):
    ...
    try:
        ...
        display_payout = int(round(preview_bet_amount(bet_amount) * multiplier))
        new_balance = add_user_balance(ctx.author.id, payout)
        ...
        embed = discord.Embed(
            title=f"Plinko - {format_preview_cash(bet_amount)} 💵",
...
class AdvancedHelpView(discord.ui.View):
    ...
    def _apply_button_styles(self):
        ...
        if hasattr(self, "vehicle_button"):
            self.vehicle_button.style = discord.ButtonStyle.secondary
    ...
    def build_vehicle_embed(self):
        ...
        embed.add_field(
            name="How to Install",
            value="Drag and drop the vehicle folder into your server's **resources** folder, then make sure the resource name matches the folder name.",
            inline=False
        )
        embed.add_field(
            name="How to Start It",
            value="Start the vehicle through **txAdmin** by using `ensure foldername` in your server config or by starting the resource from the txAdmin console.",
            inline=False
        )
        embed.add_field(
            name="Need Help?",
            value="If you need help installing the vehicle or have any questions, please open a **support ticket**.",
            inline=False
        )
        return embed
    ...
    def build_economy_embed(self):
        ...
        embed.add_field(
            name="Casino Notes",
            value=(
                f"`!mines`, `!bj`, `!plinko`, `!roulette`, `!slots`, `!crash`, `!rocket`, `!dice`, and `!baccarat` have a **{BETTING_COOLDOWN_SECONDS} second** cooldown.\n"
...
class RocketCashoutButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        display_payout = int(round(preview_bet_amount(view.bet_amount) * view.current_multiplier))
    ...
    def build_embed(self, title=None, description=None, colour=0x5865F2):
        embed = discord.Embed(
            ...
            description=description or (
                f"**{self.current_multiplier:.2f}x** Current Payout: **{format_cash(int(round(preview_bet_amount(self.bet_amount) * self.current_multiplier)))} 💵**\n\n"
                f"{self.build_graph()}\n\n"
