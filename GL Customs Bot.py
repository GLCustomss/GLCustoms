...
def round_clean_cash_value(amount):
    try:
        amount = int(amount)
    except Exception:
        return 0
    ...
    if amount < 100_000:
        ...
        step = 10_000_000
    elif amount < 100_000_000_000_000:
        step = 1_000_000_000
    else:
        step = 10_000_000_000
...
def generate_crash_point():
    ...
    if roll < 0.20:
        crash_point = random.uniform(1.25, 1.90)
    elif roll < 0.52:
        crash_point = random.uniform(1.91, 3.80)
    elif roll < 0.82:
        crash_point = random.uniform(3.81, 8.20)
    elif roll < 0.96:
        crash_point = random.uniform(8.21, 18.00)
    else:
        crash_point = random.uniform(18.01, 50.00)
...
async def slots(ctx, bet_amount=None):
    ...
    try:
        ...
        message = await send_game_premessage(
            ctx,
            f"Slots - {format_cash(round_clean_cash_value(bet_amount))} 💵",
...
async def roulette(ctx, bet_amount=None, choice=None):
    ...
    try:
        pre = await send_game_premessage(
            ctx,
            f"Roulette - {format_cash(round_clean_cash_value(bet_amount))} 💵",
            "🎡 Spinning the wheel...",
            "🔴⚫ Ball is bouncing..."
        )
...
async def crash(ctx, bet_amount=None):
    ...
    message = await send_game_premessage(
        ctx,
        f"Crash - {format_cash(round_clean_cash_value(bet_amount))} 💵",
...
async def mines(ctx, bet_amount=None, mine_count: int = 3):
    ...
    try:
        pre = await send_game_premessage(
            ctx,
            f"Mines - {format_cash(round_clean_cash_value(bet_amount))} 💵",
            "💣 Planting mines...",
            "🟩 Building your board..."
        )
...
async def bj(ctx, bet_amount=None):
    ...
    try:
        pre = await send_game_premessage(
            ctx,
            f"Blackjack - {format_cash(round_clean_cash_value(bet_amount))} 💵",
            "🃏 Shuffling deck...",
...
async def plinko(ctx, bet_amount=None):
    ...
    try:
        ...
        embed = discord.Embed(
            title=f"Plinko - {format_cash(round_clean_cash_value(bet_amount))} 💵",
