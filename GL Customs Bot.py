...
GAME_XP_PER_PLAY = 10
LIFETIME_ROLE_ID = 1359208679739494410
MAX_GIVE_AMOUNT = 100000
LIFETIME_BALANCE_THRESHOLD = 10000000000
...
PLINKO_MULTIPLIERS = [8.9, 3.0, 1.4, 1.1, 1.0, 0.5, 1.0, 1.1, 1.4, 3.0, 8.9]
...
async def check_lifetime_role(member, ctx=None):
    try:
        if member is None or member.guild is None:
            return False

        lifetime_role = member.guild.get_role(LIFETIME_ROLE_ID)
        if lifetime_role is None:
            return False

        current_balance = get_user_balance(member.id)
        has_role = lifetime_role in member.roles

        if current_balance >= LIFETIME_BALANCE_THRESHOLD and not has_role:
            await member.add_roles(lifetime_role, reason="Reached 10 billion cash")

            embed = discord.Embed(
                title="Lifetime Reward Unlocked",
                description=(
                    f"🏆 {member.mention} has reached **{format_cash(LIFETIME_BALANCE_THRESHOLD)} 💵**\n"
                    f"and unlocked their **Lifetime Access** reward."
                ),
                colour=0x57F287,
                timestamp=now_dt()
            )
            ...
            try:
                ...
                if modlog_channel and (not ctx or modlog_channel.id != getattr(ctx.channel, "id", None)):
        ...
        elif current_balance < LIFETIME_BALANCE_THRESHOLD and has_role:
            await member.remove_roles(lifetime_role, reason="Dropped below 10 billion cash")
            return False
...
async def run_weekly_leaderboard_payout():
    ...
    for index, (user_id, entry) in enumerate(top_entries, start=1):
        ...
        if member:
            await check_lifetime_role(member, ctx)
...
class MinesView(discord.ui.View):
    ...
    async def cash_out(self, interaction):
        ...
        await check_lifetime_role(self.author, self.ctx)
...
class BlackjackView(discord.ui.View):
    ...
    def build_embed(self, reveal_dealer=False, result_text=None, result_colour=0xF1C40F):
        ...
        embed = discord.Embed(
            title=f"Playing Blackjack with {format_cash(self.current_bet)} 💵",
        ...
        if result_text:
            embed.description = f"> {result_text}\n> Balance: {get_user_balance(self.author.id)} 💵"

        player_name = self.author.display_name.lower()
        embed.add_field(
            name=f"{player_name} ({player_total})",
            value=format_blackjack_hand(self.player_hand),
            inline=True
        )
        embed.add_field(
            name=f"Dealer ({dealer_total})",
            value=format_blackjack_hand(dealer_cards),
            inline=True
        )
        return embed
    ...
    async def settle_game(self, interaction):
        ...
        if dealer_total > 21 or player_total > dealer_total:
            ...
            result = f"You won {self.current_bet * 2} 💵 | Dealer busted." if dealer_total > 21 else f"You won {self.current_bet * 2} 💵"
            colour = 0x57F287
            ...
            result = f"You lost {self.current_bet} 💵"
            colour = 0xED4245
    ...
    async def hit_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if player_total > 21:
            ...
            self.finish_buttons()
            ACTIVE_GAME_USERS.discard(self.author.id)
            embed = self.build_embed(reveal_dealer=True, result_text=f"You lost {self.current_bet} 💵 | You busted.", result_colour=0xED4245)
            await interaction.response.edit_message(embed=embed, view=self)
            return
    ...
    @discord.ui.button(label="Double Down", style=discord.ButtonStyle.secondary, row=0)
    async def double_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # after double down you stand automatically
        await self.settle_game(interaction)
    ...
    async def exit_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        self.finish_buttons()
        ACTIVE_GAME_USERS.discard(self.author.id)
        embed = self.build_embed(reveal_dealer=True, result_text="Game exited. Your bet was returned.", result_colour=0x5865F2)
        await interaction.response.edit_message(embed=embed, view=self)
...
async def balance(ctx, member: discord.Member = None):
    ...
    embed = discord.Embed(
        title="Cash Balance",
        description=f"{member.mention} has **{get_user_balance(member.id)} 💵**",
        colour=0xF1C40F,
        timestamp=now_dt()
    )
...
async def give(ctx, member: discord.Member = None, amount=None):
    ...
    await check_lifetime_role(ctx.author, ctx)
    await check_lifetime_role(member, ctx)
...
async def removecash(ctx, member: discord.Member = None, amount=None):
    ...
    if amount is None:
        embed = build_case_embed("Remove Cash", "❌ Invalid amount. Use values like `100000`, `100k`, `5m`, or `1b`.", 0xE74C3C)
        await ctx.send(embed=embed)
        return
...
async def admincash(ctx, member: discord.Member = None, amount=None):
    ...
    if amount is None:
        embed = build_case_embed("Admin Cash", "❌ Invalid amount. Use values like `100000`, `100k`, `5m`, or `1b`.", 0xE74C3C)
        await ctx.send(embed=embed)
        return
...
async def mines(ctx, bet_amount=None, mine_count: int = 3):
    ...
    try:
        bet_amount = parse_cash_amount(bet_amount)
    except Exception:
        bet_amount = None

    if bet_amount is None:
        embed = build_case_embed("Mines", "❌ Invalid bet. Use values like `100000`, `100k`, `5m`, or `1b`.", 0xE74C3C)
        await ctx.send(embed=embed)
        return
...
async def bj(ctx, bet_amount=None):
    ...
    if bet_amount is None:
        await ctx.send(embed=discord.Embed(title="Blackjack", description="❌ Invalid bet. Use values like `100000`, `100k`, `5m`, or `1b`.", colour=0xE74C3C, timestamp=now_dt()))
        return
...
async def rewards(ctx):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "**Lifetime Reward**\n"
            f"Reach **{format_cash(LIFETIME_BALANCE_THRESHOLD)} 💵** to unlock the lifetime role.\n\n"
            "**Mines Jackpot**\n"
...
async def plinko(ctx, bet_amount=None):
    ...
    if bet_amount is None:
        embed = build_case_embed("Plinko", "❌ Invalid bet. Use values like `100000`, `100k`, `5m`, or `1b`.", 0xE74C3C)
        await ctx.send(embed=embed)
        return
    ...
    slot_index = random.randint(0, len(PLINKO_MULTIPLIERS) - 1)
    ...
    for _ in range(rows_count - 1):
        ...
        current_pos = max(0, min(width - 1, current_pos + move))
    ...
    def render_frame(step):
        ...
        for r in range(rows_count):
            dots_on_row = min(width, 1 + r)
            start = max(0, center - (dots_on_row // 2))
            end = min(width - 1, start + dots_on_row - 1)

            cells = []
            for c in range(width):
                if start <= c <= end:
                    if r == step and c == positions[r]:
                        cells.append("🔵")
                    else:
                        cells.append("•")
                else:
    ...
    for step in range(1, rows_count):
        await asyncio.sleep(0.22)
        embed.description = render_frame(step)
        await message.edit(embed=embed)

    await asyncio.sleep(0.18)
    embed.description = (
        render_frame(rows_count - 1) + "\n\n"
        f"You landed on **{multiplier}x**\n"
    ...
    def build_utility_embed(self):
        ...
        embed.add_field(
            ...
            value=(
                ...
                "`!balance [@user]` / `!bal [@user]` — View casino balance\n"
                "`!give @user <amount>` — Send cash\n"
...
class RocketCashoutButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await check_lifetime_role(view.author, view.ctx)
...
async def rocket(ctx, bet_amount=None):
    ...
    if bet_amount is None:
        embed = build_case_embed("Rocket", "❌ Invalid bet. Use values like `100000`, `100k`, `5m`, or `1b`.", 0xE74C3C)
        await ctx.send(embed=embed)
        return
