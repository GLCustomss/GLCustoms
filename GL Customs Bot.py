...
PLINKO_MULTIPLIERS = [4.5, 2.2, 1.45, 1.0, 0.8, 0.65, 0.8, 1.0, 1.45, 2.2, 4.5]
...
def generate_crash_point():
    ...
    if roll < 0.40:
        crash_point = random.uniform(1.15, 1.55)
    elif roll < 0.72:
        crash_point = random.uniform(1.56, 2.50)
    elif roll < 0.92:
        crash_point = random.uniform(2.51, 5.00)
    elif roll < 0.985:
        crash_point = random.uniform(5.01, 10.00)
    else:
        crash_point = random.uniform(10.01, 25.00)
    ...
    try:
        ...
        if chosen_number is not None:
            ...
            if choice_text == result_colour:
                won = True
                payout_mult = 2.08 if choice_text in {"red", "black"} else 18.0
...
async def dice(ctx, bet_amount=None, side=None):
    ...
    try:
        ...
        payout = int(round(bet_amount * 1.98)) if won else 0
        new_balance = add_user_balance(ctx.author.id, payout)
        await check_casino_car_pack_role(ctx.author, ctx)
        embed = discord.Embed(
...
async def baccarat(ctx, bet_amount=None, side=None):
    ...
    try:
        ...
        won = side == outcome
        payout_mult = {"player": 2.08, "banker": 2.02, "tie": 8.8}[outcome] if won else 0.0
        payout = int(round(bet_amount * payout_mult))
        new_balance = add_user_balance(ctx.author.id, payout)
        await check_casino_car_pack_role(ctx.author, ctx)
...
def mines_multiplier(safe_hits, mine_count, total_tiles=20):
    ...
    for _ in range(safe_hits):
        ...
        remaining_tiles -= 1
        remaining_safe -= 1
    return round(multiplier * 0.98, 2)
...
async def plinko(ctx, bet_amount=None):
    ...
    try:
        ...
        slot_index = random.choices(
            ...
            weights=[1, 2, 4, 7, 10, 10, 10, 7, 4, 2, 1],
            k=1
        )[0]
