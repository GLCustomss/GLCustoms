...
async def admincash(ctx, member: discord.Member = None, amount=None):
    ...
    amount_text = str(amount).strip().lower()

    if amount_text in {"inf", "infinite", "infinity"}:
        confirmed = await confirm_action(
            ctx,
            title="Admin Cash",
            prompt_text=f"This will set {member.mention} to **infinite cash**."
        )
        if not confirmed:
            return
        ...
        embed = discord.Embed(
            title="Admin Cash",
            description=f"✅ Set {member.mention} to **infinite cash**.

**New Balance:** {format_cash(get_user_balance(member.id))} 💵",
            colour=0x57F287,
            timestamp=now_dt()
        )
        embed.set_footer(text="GL Customs")
...
async def rewards(ctx):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "**Casino Commands**\n"
            "`!slots`, `!roulette`, `!crash`, `!rocket`, `!dice`, `!baccarat`, `!plinko`, `!mines`, `!mine`, `!bj`, `!blackjack`, `!balance`, `!bal`, `!beg`, `!work`, `!give`, `!profit`, `!mystats`, `!rank`\n\n"
            "**Mines Jackpot**\n"
...
async def vehiclehelp(ctx):
    ...
    embed = build_case_embed(
        ...
        0x3498DB,
        extra_fields=[
            ("Compatibility", "These vehicles are **FiveM ready only**.", False),
            ("ELS", "These vehicles are **non-ELS**.", False),
            ("Important", "They are only compatible with **FiveM**.", False),
            ("How to Install", "Drag and drop the vehicle folder into your server's **resources** folder, then make sure the resource name matches the folder name.", False),
            ("How to Start It", "Start the vehicle through **txAdmin** by using `ensure foldername` in your server config or by starting the resource from the txAdmin console.", False),
            ("Need Help?", "If you need help installing the vehicle or have any questions, please open a **support ticket**.", False)
        ]
    )
    await ctx.send(embed=embed)
...
class AdvancedHelpView(discord.ui.View):
    ...
    def build_rewards_embed(self):
        ...
        embed.add_field(
            name="Weekly Leaderboard Payouts",
            value=(
                f"🥇 **#1:** {format_cash(WEEKLY_REWARD_AMOUNTS[1])} 💵
"
                f"🥈 **#2:** {format_cash(WEEKLY_REWARD_AMOUNTS[2])} 💵
...
"
        ...
        embed.add_field(
            ...
            value="`!slots`, `!roulette`, `!crash`, `!rocket`, `!dice`, `!baccarat`, `!plinko`, `!mines`, `!mine`, `!bj`, `!blackjack`, `!balance`, `!bal`, `!beg`, `!work`, `!give`, `!profit`, `!mystats`, `!rank`",
            inline=False
        )
        embed.add_field(
            name="Mines Jackpot",
...
"
    ...
    @discord.ui.button(label="Vehicle Help", style=discord.ButtonStyle.secondary, row=0)
