...
async def give(ctx, *, target_and_amount: str = None):
    ...
    if ctx.author.id != OWNER_ID and amount > MAX_GIVE_AMOUNT:
...
async def profit(ctx):
    ...
    for name, stats in top_games:
        profit_val = int(stats.get("house_profit", 0))
        game_lines.append(f"`{name}` → {format_cash(profit_val)}")

    embed = discord.Embed(
        title="Casino Stats",
        description=(
            f"**House Edge**\n"
            f"`{bar}` **{house_edge * 100:.1f}%**\n\n"
            f"**Totals**\n"
...
async def plinko(ctx, bet_amount=None):
    ...
    try:
        ...
        for i, (path_row, path_col) in enumerate(reduced_path):
            await asyncio.sleep(0.003 if i < len(reduced_path) - 2 else 0.005)
