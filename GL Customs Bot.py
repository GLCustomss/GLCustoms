...
PLINKO_MULTIPLIERS = [5.0, 2.4, 1.55, 1.05, 0.82, 0.62, 0.82, 1.05, 1.55, 2.4, 5.0]
BETTING_COOLDOWN_SECONDS = 4
ROULETTE_COLOUR_PAYOUT = 2.06
ROULETTE_GREEN_PAYOUT = 38.0
ROULETTE_NUMBER_PAYOUT = 38.0
DICE_PAYOUT = 2.05
BACCARAT_PAYOUTS = {
    "player": 2.25,
    "banker": 2.19,
...
MINES_RTP = 0.995
SLOTS_REELS = ["🍒", "🍋", "🍇", "🔔", "💎", "7️⃣"]
SLOTS_WEIGHTS = [22, 20, 18, 16, 12, 12]
...
def generate_crash_point():
    edge = 1.0015
    r = random.random()
    crash_point = max(1.01, math.floor((edge / (1 - r)) * 100) / 100)
    return min(crash_point, 25.0)
    ...
    try:
        ...
        if chosen_number is not None:
            if rolled_number == chosen_number:
                won = True
                payout_mult = ROULETTE_NUMBER_PAYOUT
        else:
            if choice_text == result_colour:
                won = True
                payout_mult = ROULETTE_COLOUR_PAYOUT if choice_text in {"red", "black"} else ROULETTE_GREEN_PAYOUT
...
async def dice(ctx, bet_amount=None, side=None):
    ...
    try:
        ...
        payout = int(round(bet_amount * DICE_PAYOUT)) if won else 0
        new_balance = add_user_balance(ctx.author.id, payout)
        await check_casino_car_pack_role(ctx.author, ctx)
        embed = discord.Embed(
...
async def baccarat(ctx, bet_amount=None, side=None):
    ...
    try:
        ...
        won = side == outcome
        payout_mult = BACCARAT_PAYOUTS[outcome] if won else 0.0
        payout = int(round(bet_amount * payout_mult))
        new_balance = add_user_balance(ctx.author.id, payout)
        await check_casino_car_pack_role(ctx.author, ctx)
...
def mines_multiplier(safe_hits, mine_count, total_tiles=20):
    ...
    for _ in range(safe_hits):
        ...
        remaining_tiles -= 1
        remaining_safe -= 1
    return round(multiplier * MINES_RTP, 2)
