...
async def chess(ctx, bet_amount=None):
    ...
    if bet_amount is None:
        embed = discord.Embed(
            title="Command: !chess",
            description=(
                "**Description:** Play a fast casino chess match for cash\n"
                f"**Cooldown:** {BETTING_COOLDOWN_SECONDS} seconds\n"
                "**Usage:**\n"
                "!chess [bet]\n"
                "**Example:**\n"
    ...
    try:
        opening = random.choice([
            "Sicilian Defence", "Queen's Gambit", "King's Indian", "Ruy Lopez",
            "French Defence", "Italian Game", "Caro-Kann", "English Opening"
        ])
        opponent = random.choice([
            "Aggressive bot", "Calm grinder", "Tactical nerd", "Endgame rat",
            "Bullet demon", "Positional machine", "Chaos merchant", "Sharp calculator"
        ])
        midgame = random.choice([
            "You find a strong fork", "You castle and stabilise", "The centre opens up",
            "A queen trade happens", "A rook file opens", "The knight jumps into the attack",
            "You pressure the king side", "The position gets messy"
        ])

        outcome = random.choices(
            ["loss", "draw", "win", "checkmate"],
            weights=[42, 18, 34, 6],
            k=1
        )[0]
        ...
        payout = int(round(bet_amount * payout_map[outcome]))
        new_balance = add_user_balance(ctx.author.id, payout)
        record_house_profit("chess", bet_amount, payout, ctx.author.id)
        await check_casino_car_pack_role(ctx.author, ctx)

        embed = discord.Embed(
            title=f"Chess - {format_casino_bet_display(display_bet_amount=display_bet_amount)} 💵",
            description=(
                f"♟️ Opening: **{opening}**\n"
                f"🤖 Opponent: **{opponent}**\n\n"
                "🧠 Thinking..."
        ...
        for idx, phase in enumerate(phases):
            ...
            await message.edit(embed=embed)
            await asyncio.sleep(0.22 if idx < 2 else 0.28)

        if outcome == "checkmate":
            result_text = (
                f"♟️ Opening: **{opening}**\n"
                f"🤖 Opponent: **{opponent}**\n"
                f"🏁 Result: **Checkmate win**\n\n"
                f"✅ You crushed the board.\n"
                f"└ Won: **{format_casino_result_display(payout, new_balance, display_bet_amount)} 💵**\n"
                f"└ Balance: **{format_cash(new_balance)} 💵**"
            )
            colour = 0x57F287
            ...
            result_text = (
                ...
                f"🏁 Result: **Draw**\n\n"
                f"🟨 You held the draw.\n"
                f"└ Returned: **{format_casino_result_display(payout, new_balance, display_bet_amount)} 💵**\n"
                f"└ Balance: **{format_cash(new_balance)} 💵**"
            )
            colour = 0xF1C40F
        else:
            result_text = (
                ...
                f"🏁 Result: **Loss**\n\n"
                f"❌ You got outplayed.\n"
                f"└ Lost: **{format_casino_result_display(bet_amount, new_balance, display_bet_amount)} 💵**\n"
                f"└ Balance: **{format_cash(new_balance)} 💵**"
            )
            colour = 0xE74C3C
