...
async def vehiclehelp(ctx):
    ...
    embed = build_case_embed(
        ...
        extra_fields=[
            ...
            ("How To Install", "1. Drag vehicle folder into **resources**.\n2. Start it in **txAdmin** or add `ensure foldername` to server.cfg.\n3. Restart server.", False),
...
class AdvancedHelpView(discord.ui.View):
    ...
    def build_vehicle_embed(self):
        embed = discord.Embed(
            title="🚗 Vehicle Help",
            description="Compatibility and install help for GL Customs vehicles.",
            colour=0x3498DB,
            timestamp=now_dt()
        )
        embed.set_author(name="GL Customs", icon_url=LOGO_URL)
        embed.set_footer(text="GL Customs", icon_url=LOGO_URL)
        embed.add_field(name="Compatibility", value="These vehicles are **FiveM ready only** and **non-ELS**.", inline=False)
        embed.add_field(name="Install Guide", value="1. Drag the vehicle folder into your server **resources** folder.\n2. Add `ensure foldername` to server.cfg or start it in **txAdmin**.\n3. Restart server or start resource in txAdmin.", inline=False)
        embed.add_field(name="Important", value="Only compatible with **FiveM** servers.", inline=False)
        return embed
    ...
    @discord.ui.button(label="Vehicle", style=discord.ButtonStyle.secondary, row=0)
