...
def set_user_balance(user_id, amount):
    ...
    balances = data.setdefault("balances", {})
    amount = int(amount)
    if amount >= INFINITE_CASH_VALUE:
        balances[str(user_id)] = INFINITE_CASH_VALUE
    else:
...
def add_user_balance(user_id, amount):
    ...
    if is_infinite_balance(current):
        return current
...
def is_infinite_balance(amount):
    try:
...
def format_cash(amount):
    ...
    if is_infinite_balance(numeric_amount):
        return "∞"

    negative = numeric_amount < 0
    amount = abs(float(numeric_amount))
