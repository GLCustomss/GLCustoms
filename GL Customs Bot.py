...
def record_house_profit(game_name, wager, payout):
    try:
        wager = int(wager)
        payout = int(payout)
    except Exception:
...
async def slots(ctx, bet_amount=None):
    ...
    try:
        ...
        record_house_profit("baccarat", bet_amount, payout)
        record_house_profit("roulette", bet_amount, payout)
        record_house_profit("slots", bet_amount, payout)
...
class CrashGameView(discord.ui.View):
    ...
    async def cashout_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        record_house_profit("crash", self.bet_amount, payout)
    ...
    async def run(self):
        while not self.finished:
            ...
            if self.current_multiplier >= self.crash_point:
                ...
                self.finished = True
                record_house_profit("crash", self.bet_amount, 0)
                ACTIVE_GAME_USERS.discard(self.author.id)
                for child in self.children:
                    child.disabled = True
                if self.message:
...
async def dice(ctx, bet_amount=None, side=None):
    ...
    try:
        ...
        new_balance = add_user_balance(ctx.author.id, payout)
        record_house_profit("dice", bet_amount, payout)
        await check_casino_car_pack_role(ctx.author, ctx)
        embed = discord.Embed(
...
class MinesView(discord.ui.View):
    ...
    async def reveal_tile(self, interaction, index):
        ...
        if index in self.mine_positions:
            ...
            if protected_pick:
                ...
                ACTIVE_GAME_USERS.discard(self.author.id)
                record_house_profit("mines", self.bet_amount, 0)
                embed = self.build_embed(
                    title="Game Over – You hit a mine",
                    description=f"You lost {format_cash(round_mines_display_amount(self.bet_amount))} 💵",
                    colour=0xED4245
                )
                await interaction.response.edit_message(embed=embed, view=self)
                return
    ...
    async def cash_out(self, interaction):
        ...
        record_house_profit("mines", self.bet_amount, total_payout)
        await check_casino_car_pack_role(self.author, self.ctx)
...
async def rewards(ctx):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "**Casino Commands**\n"
            "`!slots`, `!roulette`, `!crash`, `!rocket`, `!dice`, `!baccarat`, `!plinko`, `!mines`, `!bj`\n\n"
            "**Mines Jackpot**\n"
...
async def profit(ctx):
    if not has_regular_member_permission(ctx.author):
        embed = build_case_embed("Profit Dashboard", "❌ You do not have permission to use this command.", 0xE74C3C)
        await ctx.send(embed=embed)
        return
    ...
    total_house_profit = sum(int(v.get("house_profit", 0)) for v in totals.values())
    total_plays = sum(int(v.get("plays", 0)) for v in totals.values())

    edge_ratio = (total_house_profit / total_wagered) if total_wagered > 0 else 0.0
    rtp_ratio = (total_paid_out / total_wagered) if total_wagered > 0 else 0.0

    recent_days = history[-10:]
    day_values = [max(0, int(item.get("house_profit", 0))) for item in recent_days]
    sparkline = build_profit_sparkline(day_values)
    edge_bar = build_profit_bar(min(max(edge_ratio, 0.0), 0.20) / 0.20, width=20)

    top_games = sorted(
        totals.items(),
        key=lambda item: int(item[1].get("house_profit", 0)),
        reverse=True
    )[:6]

    game_lines = []
    for game_name, stats in top_games:
        wagered = int(stats.get("wagered", 0))
        paid_out = int(stats.get("paid_out", 0))
        profit_value = int(stats.get("house_profit", 0))
        plays = int(stats.get("plays", 0))
        local_edge = (profit_value / wagered * 100) if wagered > 0 else 0.0
        game_lines.append(
            f"`{game_name}` • plays {plays} • wagered {format_cash(wagered)} • paid {format_cash(paid_out)} • edge {local_edge:.2f}%"
        )

    history_lines = []
    peak = max(day_values) if day_values else 1
    for item in recent_days:
        profit_value = int(item.get("house_profit", 0))
        bar = build_profit_bar((max(profit_value, 0) / peak) if peak else 0, width=10)
        history_lines.append(f"`{item.get('date')}` {bar} {format_cash(profit_value)}")

    embed = discord.Embed(
        title="Profit Dashboard",
        description=(
            f"**House Edge Progress**\n"
            f"`{edge_bar}` **{edge_ratio * 100:.2f}%**\n\n"
            f"**Technical Snapshot**\n"
            f"• Total wagered: **{format_cash(total_wagered)} 💵**\n"
            f"• Total paid out: **{format_cash(total_paid_out)} 💵**\n"
            f"• House profit: **{format_cash(total_house_profit)} 💵**\n"
            f"• RTP: **{rtp_ratio * 100:.2f}%**\n"
            f"• Total tracked plays: **{total_plays}**\n\n"
            f"**10-Day Profit Graph**\n"
            f"```{sparkline}```"
        ),
        colour=0x3498DB,
        timestamp=now_dt()
    ...
    embed.set_author(name="GL Customs", icon_url=LOGO_URL)
    embed.set_footer(text="GL Customs Analytics", icon_url=LOGO_URL)

    embed.add_field(
        name="Top Game Breakdown",
        value="\n".join(game_lines) if game_lines else "No tracked casino data yet.",
        inline=False
    )
    embed.add_field(
        name="Daily Profit Timeline",
        value="\n".join(history_lines) if history_lines else "No daily history yet.",
        inline=False
    )
...
async def ban(ctx, target_input: str = None, *, reason="No reason provided"):
    ...
    if target_id == ctx.author.id:
        embed = build_case_embed("Ban", "❌ You cannot ban yourself.", 0xE74C3C, extra_fields=[("User ID", str(target_id), True)])
        await ctx.send(embed=embed)
        return

    if target_id == bot.user.id:
        embed = build_case_embed("Ban", "❌ You cannot ban the bot.", 0xE74C3C, extra_fields=[("User ID", str(target_id), True)])
        await ctx.send(embed=embed)
        return
    ...
    try:
        ...
        embed = build_case_embed("Ban", "❌ I do not have permission to ban that user.", 0xE74C3C, extra_fields=[("User ID", str(target_id), True)])
        await ctx.send(embed=embed)
        return
    except discord.HTTPException:
        embed = build_case_embed("Ban", "❌ I could not ban that user ID.", 0xE74C3C, extra_fields=[("User ID", str(target_id), True)])
        await ctx.send(embed=embed)
        return
    ...
    embed = build_case_embed(
        ...
        user=target_member,
        reason=reason,
        extra_fields=[("User ID", str(target_id), True)] if target_member is None else None
    )
...
async def gban(ctx, target_input: str = None, *, reason="No reason provided"):
    ...
    if target_id == ctx.author.id:
        embed = build_case_embed("Global Ban", "❌ You cannot globally ban yourself.", 0xE74C3C, extra_fields=[("User ID", str(target_id), True)])
        await ctx.send(embed=embed)
        return

    if target_id == bot.user.id:
        embed = build_case_embed("Global Ban", "❌ You cannot globally ban the bot.", 0xE74C3C, extra_fields=[("User ID", str(target_id), True)])
        await ctx.send(embed=embed)
        return
    ...
    if target_member is None:
        extra_fields.insert(0, ("User ID", str(target_id), True))
...
async def plinko(ctx, bet_amount=None):
    ...
    try:
        ...
        new_balance = add_user_balance(ctx.author.id, payout)
        record_house_profit("plinko", bet_amount, payout)
        await check_casino_car_pack_role(ctx.author, ctx)
...
class RocketCashoutButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        record_house_profit("rocket", view.bet_amount, payout)
...
class RocketView(discord.ui.View):
    ...
    async def run_game(self):
        while not self.finished:
            ...
            if self.current_multiplier >= self.crash_point:
                self.finished = True
                record_house_profit("rocket", self.bet_amount, 0)
                self.stop_game()
                embed = self.build_embed(
