...
def build_eightball_answer(question_text):
    ...
    opener = random.choice(EIGHTBALL_TOPIC_OPENERS[topic])
    outcome_line = random.choice(EIGHTBALL_OUTCOME_LINES[outcome])
    tone_line = random.choice(EIGHTBALL_TONE_LINES[outcome])
    followup = random.choice(EIGHTBALL_FOLLOWUPS[outcome])
    advice = random.choice(EIGHTBALL_TOPIC_ADVICE[topic])
    structures = [
        f"{opener} {outcome_line} {tone_line} It feels like {focus}. {followup} Also, {advice}.",
        f"{opener} when it comes to {focus}, {outcome_line} {tone_line} {followup}",
        f"{outcome_line} {opener.lower()} it feels like {focus}. {tone_line} Also, {advice}.",
        f"{opener} I am getting {outcome_line.lower()} around {focus}. {tone_line} {followup}",
        f"{opener} {focus} is giving me {outcome_line.lower()} energy. {tone_line} Also, {advice}.",
        f"{outcome_line} For {focus}, {tone_line} {followup} Also, {advice}.",
        f"{opener} the answer around {focus} is {outcome_line.lower()} {tone_line} {followup}",
        f"{opener} I would say {outcome_line.lower()} for {focus}. {tone_line} Also, {advice}."
    ]
...
async def eightball(ctx, *, question: str = None):
    ...
    colour_map = {"yes": 0x57F287, "maybe": 0xF1C40F, "no": 0xE74C3C}
    embed = game_embed(
        "🎱 Magic 8 Ball",
        f"**Question:** {question}\n\n**Answer:** {answer}",
        colour_map.get(outcome, 0x5865F2)
    )
    embed.add_field(name="Topic", value=topic.title(), inline=True)
    embed.add_field(name="Reading", value=outcome.title(), inline=True)
    await ctx.send(embed=embed)
