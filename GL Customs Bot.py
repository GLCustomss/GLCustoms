...
GAME_COMMAND_NAMES = {
    ...
    "reverse", "say", "fact", "memeidea", "scramble", "achievement", "namegen", "mines", "mine", "bj", "blackjack", "balance", "bal", "beg", "work", "give"
}
...
class MinesView(discord.ui.View):
    def __init__(self, author, bet_amount, mine_count):
    ...
    def build_embed(self, title=None, description=None, colour=0xF1C40F):
        embed = discord.Embed(
            title=title or f"Mines - {self.bet_amount} 💵",
    ...
    async def reveal_tile(self, interaction, index):
        ...
        embed = self.build_embed(
            description=(
                ...
                f"**Cash Out Value:** {current_cashout} 💵"
            ),
            colour=0xF1C40F
        )
        await interaction.response.edit_message(embed=embed, view=self)
...
class BlackjackView(discord.ui.View):
    ...
    def build_embed(self, reveal_dealer=False, result_text=None, result_colour=0xF1C40F):
        ...
        embed = discord.Embed(
            title=f"Playing Blackjack with {self.current_bet} 💵",
...
async def give(ctx, member: discord.Member = None, amount: int = None):
    ...
    if not ok:
        embed = discord.Embed(
            title="Give",
            description=f"❌ You only have {current_balance} 💵",
            colour=0xE74C3C,
            timestamp=now_dt()
        )
        embed.set_footer(text="GL Customs", icon_url=LOGO_URL)
    ...
    embed = discord.Embed(
        description=(
            f"✅ You gave **{amount} 💵** to {member.display_name}\n"
            f"└ Balance: **{get_user_balance(ctx.author.id)} 💵**"
        ),
        colour=0xF1C40F,
        timestamp=now_dt()
    )
...
async def admincash(ctx, member: discord.Member = None, amount: int = None):
...
async def mines(ctx, bet_amount: int = None, mine_count: int = 3):
    if not has_regular_member_permission(ctx.author):
    ...
    if not ok:
        await ctx.send(embed=discord.Embed(title="Mines", description=f"❌ You only have {current_balance} 💵", colour=0xE74C3C, timestamp=now_dt()))
        return

    ACTIVE_GAME_USERS.add(ctx.author.id)
    try:
        view = MinesView(ctx.author, bet_amount, mine_count)
...
async def bj(ctx, bet_amount: int = None):
    ...
    if ctx.author.id in ACTIVE_GAME_USERS:
        embed = discord.Embed(
            ...
            description=(
                "**Description:** Play blackjack and win money\n"
                "**Cooldown:** 3 seconds\n"
                "**Usage:**\n"
                "!bj [bet]\n"
                "**Example:**\n"
    ...
    ok, current_balance = remove_user_balance(ctx.author.id, bet_amount)
    if not ok:
        await ctx.send(embed=discord.Embed(title="Blackjack", description=f"❌ You only have {current_balance} 💵", colour=0xE74C3C, timestamp=now_dt()))
        return
