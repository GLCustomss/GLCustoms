...
def format_mines_bet_display(amount):
    try:
        ...
        return format_cash(amount)

    if is_infinite_cash(value):
        return format_cash(INFINITE_GAME_DISPLAY_BET)
    return format_cash(value)


def format_mines_cashout_result(amount, balance_reference=None):
    try:
        value = int(amount)
    except Exception:
        return format_cash(amount)

    if balance_reference is not None and is_infinite_cash(balance_reference) and value > 0:
        return "∞"
    return format_cash(round_mines_display_amount(value))
...
async def slots(ctx, bet_amount=None):
    ...
    try:
        bet_amount = resolve_game_bet_amount(ctx.author.id, bet_amount)
    except Exception:
        bet_amount = None
...
class MinesView(discord.ui.View):
    ...
    def build_embed(self, title=None, description=None, colour=0xF1C40F):
        embed = discord.Embed(
            title=title or f"Mines - {format_mines_bet_display(self.bet_amount)} 💵",
    ...
    async def cash_out(self, interaction):
        ...
        embed = self.build_embed(
            ...
            description=f"You won {format_mines_cashout_result(total_payout, updated_balance)} 💵\n\n**Balance:** {format_cash(updated_balance)} 💵",
            colour=0x57F287
        )
        await interaction.response.edit_message(embed=embed, view=self)
